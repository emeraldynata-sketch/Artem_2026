(function () {
  ...
  function parseRoute() {
    ...
    return parts[0] === "direction" && parts[1]
      ? { view: "direction", key: decodeURIComponent(parts[1]) }
      : { view: "summary" };
  }
  ...
  function renderSummary(search) {
    pageTitle.textContent = `Свод ${data.vuz}`;
    pageSubtitle.textContent = `Код ${data.applicantName}: ${applicantId}. Балл ${data.vuz}: ${data.applicantScore}; красным отмечены отсечки ${data.applicantScore} и выше.`;
    ...
    summaryView.innerHTML = `
      <div class="cards">
        ${metric(`Направления ${data.vuz}`, data.directions.length, "выгрузки Госуслуг", `Сколько направлений ${data.vuz} есть в загруженных списках ${data.applicantName}.`)}
        ${metric("Строки в списках", data.totalRows, "после обогащения", `Общее число строк во всех загруженных направлениях ${data.vuz}.`)}
        ${metric("Лучшая отсечка: основной без чужих", bestMain ? bestMain.mainCutoff : "", bestMain ? bestMain.direction : "", "Минимальный проходной балл по модели основного приоритета, исключая известных абитуриентов с согласием в другом вузе.")}
        ${metric("Лучшая отсечка: высший проходной", bestHigh ? bestHigh.highCutoff : "", bestHigh ? bestHigh.direction : "", "Минимальный проходной балл по модели приказа: учитываются только согласия и прохождение по приоритетам.")}
      </div>
      <div class="panel">
        <div class="panel-header">
          <div class="panel-title">Свод по направлениям ${escapeHtml(data.vuz)}</div>
        </div>
  ...
  function renderDirection(direction, search) {
    ...
    document.querySelector("#directionView .cards .metric:nth-child(2)")
      ?.insertAdjacentHTML("afterend", metric("Осн. без высшего и без согласий", direction.mainWithoutHighNoConsent, `выше ${data.applicantName}`, "Абитуриенты с основным приоритетом, без высшего проходного и без известного согласия."));
  ...
  function safeFileName(value) {
    return String(value || "rating").replace(/[\\/:*?"<>|]+/g, "_").replace(/\s+/g, " ").trim().slice(0, 90) || "rating";
  }
